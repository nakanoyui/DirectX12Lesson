...
std::shared_ptr<GLTFModel> LoadGLTFModel(const std::string& path)
{
	...
	if (model.skins.size() > 0)
	{
		...
		for (UINT ji = 0; ji < model.skins[0].joints.size(); ji++)
		{
			...
			GLTFNode* boneNode = &destModel->Nodes[nodeIdx];
	...
	for (UINT nodei = 0; nodei < destModel->Nodes.size(); nodei++)
	{
		...
		for (UINT pi = 0; pi < tempPrimitives.size(); pi++)
		{
			...
			if (prim->Vertices.size() >= 1) {
				UINT st = static_cast<UINT>(destNode->Mesh.Vertices.size());
				destNode->Mesh.Vertices.resize(destNode->Mesh.Vertices.size() + prim->Vertices.size());
				memcpy(&destNode->Mesh.Vertices[st], &prim->Vertices[0], prim->Vertices.size() * sizeof(ModelVertex));
			}

			// インデックス合成
			if (prim->Faces.size() >= 1) {
				UINT st = static_cast<UINT>(destNode->Mesh.Faces.size());
			...
			// Subset
			destNode->Mesh.Subsets[pi].FaceCount += static_cast<UINT>(prim->Faces.size());	// 面数を加算

			// 
			currentVertexIdx += static_cast<UINT>(prim->Vertices.size());
			currentFaceIdx += static_cast<UINT>(prim->Faces.size());
