...
{
	m_wpAssetManager.lock()->GetShader("PointShader")->Begin();
	...
	Application::Instance().GetGameProcess()->GetGameManager()->GetCameraManager()->Set3D(1);
	...
	if (!pTex)
	{
		m_wpAssetManager.lock()->GetTexture(Application::Instance().GetGameProcess()->GetGameManager()->GetJsonManager()->GetAssetJson().at("Texture").at("Particle").at("Default"))->Set(3);
	}
	else
	{
		pTex->Set(3);
