...
public:
	...
	std::shared_ptr<TPSCamera> GetCamera(const std::string_view& cameraName);
	std::shared_ptr<TPSCamera> GetNowCamera() { return m_spNowCamera; }

	void SetCamera(std::shared_ptr<TPSCamera>& camera);
	void SetMatrix(Math::Matrix& mat);
