...
void Robot::LoadAsset()
{
	...
	m_wpModelManager.lock()->ResisterModelWork(m_wpJsonManager.lock()->GetAssetJson().at("Robot").at("Model"));
	m_spModelWork = m_wpModelManager.lock()->GetModelWork("Robot.gltf");
...
void Robot::DrawEffect()
{
	...
	m_wpShaderManager.lock()->GetShader("EffectShader")->DrawEffect(m_upSearchDebugWireFrame->GetMesh().get(), m_transform.Matrix());

	m_wpShaderManager.lock()->GetShader("EffectShader")->DrawEffect(m_upAttackDebugWireFrame->GetMesh().get(), m_transform.Matrix());
	...
	m_wpShaderManager.lock()->GetShader("EffectShader")->DrawEffect(m_upLHandDebugWireFrame->GetMesh().get(), mLocal * mRotate * mTrans);
...
void Robot::UpdateCollision()
{
	...
	DirectX::BoundingBox lHandaabb;
	lHandaabb.Transform(lHandaabb, mLocal * mRotate * mTrans);
	lHandaabb.Extents = Math::Vector3(
	...
	DirectX::BoundingBox rHandaabb;
	rHandaabb.Transform(rHandaabb, mLocal * mRotate * mTrans);
	rHandaabb.Extents = Math::Vector3(
		m_wpJson.lock()->GetConstJson().at("CubeSize").at("X").get<float>(),
		m_wpJson.lock()->GetConstJson().at("CubeSize").at("Y").get<float>(),
		m_wpJson.lock()->GetConstJson().at("CubeSize").at("Z").get<float>());

	if (rHandaabb.Intersects(bs))
	{
		if (m_wpPlayerManager.lock()->GetPlayer().lock()->GetDamageType() != DamageType::KnockBack01)
		{
			m_wpPlayerManager.lock()->GetPlayer().lock()->SetDamageType(DamageType::KnockBack01);
