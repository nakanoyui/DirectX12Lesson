...
void Shader::Begin(bool wireFrame)
{
...
void Shader::DrawEffect(Mesh* pMesh, const Math::Matrix& mWorld,const Math::Vector3& offset)
{
	if (pMesh == nullptr)return;

	// 座標情報以外いらないから捨てる
	Math::Matrix mTrans = Math::Matrix::CreateTranslation(
		{ mWorld.Translation().x+ offset.x, mWorld.Translation().y + offset.y, mWorld.Translation().z+ offset.z });

	GraphicsDevice::Instance().GetCBufferAllocater()->BindAndAttach(2, ConstantBuffer::cbWorld{ mTrans });
