	...
	for (const auto& wpEnemy : Application::Instance().GetGameSystem()->GetEnemyManager()->GetEnemyObjects())
	{
		if (wpEnemy.lock()->GetCollider()->Intersects(Collider::Type::TypeAttack, aabb, nullptr))
		{
			...
			case Player::AnimeType::Combo01:
				wpEnemy.lock()->SetDatameType(EnemyObject::DamageType::KnockBack1);
				break;
			case Player::AnimeType::Combo02:
				wpEnemy.lock()->SetDatameType(EnemyObject::DamageType::KnockBack2);
				break;
			case Player::AnimeType::Combo03:
				wpEnemy.lock()->SetDatameType(EnemyObject::DamageType::BlowAway);
				break;
			default:
				break;
			}
		...
		{
			wpEnemy.lock()->SetDatameType(EnemyObject::DamageType::None);
		}
	}
}
