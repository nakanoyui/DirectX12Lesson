...
void Sword::LoadAsset()
{
	...
	m_spModelWork = m_wpAssetManager.lock()->GetModelWork(m_wpJsonManager.lock()->GetAssetJson().at("Sword").at("Model"));
}
...
void Sword::DrawEffect()
{
	...
	m_wpAssetManager.lock()->GetShader("EffectShader")->DrawEffect(m_upDebugWireFrame->GetMesh().get(), mLocal * mRotate * mTrans);
}
...
void Sword::ImGuiUpdateGameObject()
{
}
