...
void Mesh::Create(GraphicsDevice* pGraphicsDevice, const std::vector<ModelVertex>& vertices, const std::vector<MeshFace>& faces, const std::vector<MeshSubset>& subsets, bool bSkinMesh)
{
	...
	resdesc.Width = static_cast<UINT>(sizeof(ModelVertex) * (vertices).size());
	resdesc.Height = 1;
	...
	m_vbView.StrideInBytes = sizeof(ModelVertex);
