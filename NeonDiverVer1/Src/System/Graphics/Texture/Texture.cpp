...
bool Texture::Load(const std::string& filepath, GraphicsDevice* pGraphicsDevice)
{
	...
	MeshData<TextureVertex>	m_texVert;
	MeshCreater::Create2DQuad(m_texVert, { float(metadata.width),float(metadata.height)});
	m_spMesh = std::make_shared<Mesh>();
	m_spMesh->Create(&GraphicsDevice::Instance(), m_texVert);
