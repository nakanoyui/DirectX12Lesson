...
{
	m_bbIdx = m_pSwapChain->GetCurrentBackBufferIndex();
	auto rtvH = m_upSwapChainRT->GetHeap()->GetCPUDescriptorHandleForHeapStart();
	rtvH.ptr += m_bbIdx * m_pDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
	auto dsvH = m_pDsvHeap->GetCPUDescriptorHandleForHeapStart();

	SetResourceBarrier(m_pFrameBuffers[m_bbIdx], D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
	...
	m_pCmdList->RSSetScissorRects(1, &m_rect);
}

void GraphicsDevice::PostDraw()
{
	SetResourceBarrier(m_pFrameBuffers[m_bbIdx], D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
