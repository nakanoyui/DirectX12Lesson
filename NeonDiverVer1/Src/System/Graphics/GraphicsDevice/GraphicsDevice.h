...
public:
	...
	// 白テクスチャ取得
	Microsoft::WRL::ComPtr<ID3D12Resource> GetWhiteTex() const { return m_pWhiteTex; }
...
public:
	...
	std::shared_ptr<CBVSRVUAVHeap> GetCBVSRVUAVHeap() { return m_spCBVSRVUAVHeap; }

	/// <summary>
	/// 定数バッファアロケーターのポインタを返す関数
	/// </summary>
	/// <returns>定数バッファアロケーターのポインタ</returns>
	std::shared_ptr<CBufferAllocater> GetCBufferAllocater() { m_spCBVSRVUAVHeap->Set(); return m_spCBufferAllocater; }
