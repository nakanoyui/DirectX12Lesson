...
void GPUParticleEmitter::Draw(ComputePipeline* pComputePipeline, const Math::Matrix& mat, Texture* pTex)
{
	...
	if (!pTex)
	{
		m_wpAssetManager.lock()->GetTexture(Application::Instance().GetSceneManager()->GetCurrentScene()->GetJsonManager()->GetAssetJson().GetConstJson().at("Texture").at("Particle").at("Default"))->Set(3);
	}
	else
	{
		pTex->Set(3);
