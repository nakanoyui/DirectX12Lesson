...
bool Collider::Intersects(Type checkType, const DirectX::BoundingSphere& target, std::list<Collider::CollisionResult>* pResults)
{
	...
	for (auto& collisionShape : m_spCollisions)
	{
		...
		if (collisionShape->Intersects(target, m_wpTransformComponent.lock()->GetMatrix(), pTmpRes))
		{
			isHit = true;
...
bool BoxCollision::Intersects(const DirectX::BoundingSphere& target, const Math::Matrix& world, Collider::CollisionResult* pRes)
{
	DirectX::BoundingBox myBox;

	m_box.Transform(myBox, world);
