...
void Shader::Create(const std::vector<InputLayout>& inputLayouts, const std::vector<RangeType>& rangeTypes, BlendMode blendMode, D3D12_PRIMITIVE_TOPOLOGY_TYPE topologyType, bool bDepth, bool bDepthMask, int rtvCount, bool alphaToCovarage)
{
	...
	if (!SetIsUseDepth(bDepth)) { assert(0 && "デプスフラグのセットに失敗しました"); }
	if (!SetIsUseDepthMask(bDepthMask)) { assert(0 && "デプスマスクフラグのセットに失敗しました"); }
...
bool Shader::LoadShaderFile(const std::wstring& filepath)
{
	...
	if (FAILED(result))
	{
		assert("ピクセルシェーダーのコンパイルに失敗しました");
	}

	fullFilepath = currentPath + filepath + L"_GS" + format;
	result = D3DCompileFromFile(fullFilepath.c_str(), nullptr, include, "main", "gs_5_0", flag, 0, &m_pGSBlob, &errorBlob);
...
bool Shader::SetIsUseDepthMask(bool bDepthMask)
{
...
void Shader::CreatePipeline()
{
	...
	m_pPipeline->Create({ m_pVSBlob,m_pPSBlob,m_pGSBlob,m_pHSBlob,m_pDSBlob }, m_bDepth, m_bDepthMask, m_rtvCount, m_bAlphaToCoverageEnable);
}
...
void Shader::DrawTex(Texture* pTexture, Math::Matrix mWorld)
{
	...
	{
		Application::Instance().GetSceneManager()->GetCurrentScene()->GetAssetManager()->GetMesh("2DQuad")->DrawInstanced();
	}
}
...
void Shader::DrawEffect(Mesh* pMesh, const Math::Matrix& mWorld)
{
	if (pMesh == nullptr)return;
