#include "inc_PlayerShader.hlsli"
...
Texture2D g_dissolveMap : register(t4); // ディゾルブマップ
...
{
    float dissolveCol = g_dissolveMap.Sample(g_ss, In.UV).r;

    if (dissolveCol < g_dissolveRate)
    {
        discard;
    }
    ...
    float diffuseFromFresnel = CalcDiffuseFromFresnel(normal, -g_DL_Dir.rgb, vCam);
    ...
    float3 spec = CookTorranceSpecular(-g_DL_Dir.rgb, vCam, normal, smooth) * g_DL_Color.rgb;
    ...
    float rim = 1.0f - abs(dot(vCam, normal));
    float3 emission = g_limColor * pow(abs(rim), g_limPower) * g_limPower;
    lig += emission;
