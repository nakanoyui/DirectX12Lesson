...
{
	m_transform.Positon(ImGuiHelper::CheckDataVector3(m_pGameObject->GetJsonManager().lock()->GetJson(m_objectName)->WorkConstJson(),
		Math::Vector3(m_transform.Positon().x, m_transform.Positon().y, m_transform.Positon().z), "Position", Math::Vector3()));

	m_transform.RotationEuler(ImGuiHelper::CheckDataVector3(m_pGameObject->GetJsonManager().lock()->GetJson(m_objectName)->WorkConstJson(),
		m_euler, "Rotation", Math::Vector3()));

	m_transform.Scale(ImGuiHelper::CheckDataVector3(m_pGameObject->GetJsonManager().lock()->GetJson(m_objectName)->WorkConstJson(),
