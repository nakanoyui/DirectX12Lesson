...
void ImGuiManager::Init()
{
	...
	m_incrementSize = GraphicsDevice::Instance().GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
D3D12_CPU_DESCRIPTOR_HANDLE ImGuiManager::GetCPUHandle(UINT offset)
{
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle = m_pHeap->GetCPUDescriptorHandleForHeapStart();
	cpuHandle.ptr += (m_incrementSize * offset);
	return cpuHandle;
}

D3D12_GPU_DESCRIPTOR_HANDLE ImGuiManager::GetGPUHandle(UINT offset)
{
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = m_pHeap->GetGPUDescriptorHandleForHeapStart();
	gpuHandle.ptr += (m_incrementSize * offset);
	return gpuHandle;
}