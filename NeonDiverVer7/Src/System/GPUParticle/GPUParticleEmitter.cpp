...
void GPUParticleEmitter::Create(GPUParticleManager* pManager, UINT useParticleValue)
{
	...
	for (int i = 0; i < m_useParticleValue; ++i)
	{
...
{
	Application::Instance().GetGameSystem()->GetShaderManager()->GetShader("PointShader")->Begin();
	...
	Application::Instance().GetGameSystem()->GetCameraManager()->Set3D(1);
	...
	if (!pTex)
	{
		Application::Instance().GetGameSystem()->GetTextureManager()->GetTexture("Particle.png")->Set(3);
	}
	else
	{
		pTex->Set(3);
	...
	Application::Instance().GetGameSystem()->GetMeshManager()->GetMesh("Point")->DrawInstanced(m_useParticleValue);
