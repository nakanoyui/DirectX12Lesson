...
bool Texture::Load(const std::string& filepath, GraphicsDevice* pGraphicsDevice, bool bCreateMesh)
{
	...
	if (bCreateMesh)
	{
		...
		MeshCreater::Create2DQuad(texVert, { static_cast<float>(metadata.width) ,static_cast<float>(metadata.height) });
